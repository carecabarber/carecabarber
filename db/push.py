...
from db._conn import _read, _write, FMT, _agora


def push_guardar(barbeiro_id, barbearia_id, endpoint, p256dh, auth):
...
def push_remover(endpoint):
    with _write() as conn:
        conn.execute("DELETE FROM push_subscriptions WHERE endpoint=?", (endpoint,))


def push_listar(barbearia_id, barbeiro_id=None):
...
def push_remover_expiradas(endpoints_invalidos):
